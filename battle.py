...
class Battle:
    ...
    def __after_attack_logics(self, attacking_monster: MonsterBase, attacked_monster: MonsterBase, attacking_team: MonsterTeam, attacked_team: MonsterTeam, ref_num: int):
        ...
        :parameter attacking_monster: The monster that attacked
        :parameter attacked_monster: The monster that was attacked
        :parameter attacking_team: The team of the monster that was attacking
        :parameter attacked_team: The team of the monster that was attacked
        :parameter ref_num: The reference number of the team that was attacked 
        """
        ...
        if (attacked_monster.get_hp()) <= 0: #if attacked monster died
            ...
            '''RESULT LOGIC-----------------------------------------------------------------------------------------------------------------------------'''
            if ((attacked_team.team.is_empty()) and (attacking_team.team.is_empty()) and (attacking_monster()<=0) and (attacked_monster()<=0)): # if the last two monsters attacked at the same time and killed each other at same time it's a draw)
                self.result = self.Result.DRAW # draw the result

            elif (attacked_team.team.is_empty()) and (attacked_monster()<=0) and (ref_num==1) : #if attacked team is empty and their last monster on field just died
                    self.result = self.Result.TEAM2 # team2 wins
            
            elif (attacking_team.team.is_empty()) and (attacking_monster()<=0) and (ref_num==2) :
    ...
    def process_turn(self) -> Optional[Battle.Result]:
        ...
        * return the battle result if completed.
        ...
        if not monster1_dead and not monster2_dead: #subtract 1 hp from both monsters if they both survive
            ...
            monster1 = self.out1
            monster2 = self.out2
            self.__after_attack_logics(monster1, monster2, self.team1, self.team2, 2, True)
            self.__after_attack_logics( monster2, monster1, self.team2, self.team1, 1, True)
        ...
        if self.team1.choose_action(self.out1, self.out2) == self.Action.SWAP: # Swaps the monster on field with the one retrieve from the team (for team1)
            ...
            self.team2.add_to_team(monster_onField)
        '''---------------------------------------------------------------------------------------------------------------------------------------------------------------'''
        ...
        if (self.team1.choose_action(self.out1, self.out2) == self.Action.ATTACK) and (self.team2.choose_action(self.out2, self.out1) == self.Action.ATTACK): #if both teams attack at same time 
            ...
            if (self.out1.get_speed() > self.out2.get_speed()): # if first monster speed faster than second then the first monster attacks first
                ...
                monster2_dead = self.__after_attack_logics(self.out2, self.team1, self.team2, 2, self.out1) #check if the monster has been killed (Boolean) 
    ...
    def battle(self, team1: MonsterTeam, team2: MonsterTeam) -> Battle.Result:
        ...
        print(f'Congrats {result} you won!')
